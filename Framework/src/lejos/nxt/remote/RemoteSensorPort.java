...
public class RemoteSensorPort implements NXTProtocol, ADSensorPort, I2CPort {
	 ...
	 * 
	 * @param address
	 *            the I2C address (x01 - x7F)
	 * @param writeBuffer
	 *            data to write
	 * @param writeOffset
	 *            offset of the data in the write buffer
	 * @param writeLength
	...
	public int i2cStart(int address, byte[] writeBuffer, int writeOffset,
			int writeLength, int readLength) {
	...
	public synchronized int i2cTransaction(int deviceAddress, byte[] writeBuf,
			...
			int readLen) {
		int ret = i2cStart(deviceAddress, writeBuf, writeOffset, writeLen,
				readLen);
		if (ret < 0)
			return ret;
		i2cWaitIOComplete();
	...
	@Override
	public <SensorType extends Sensor> SensorType getSensor(
			Class<SensorType> sensorType) {
		throw new NotImplementedException("RemoteSensorPort.getSensor"); // FIXME
	}
}
