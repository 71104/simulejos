...
public class Sound {
	...
	/**
	 * Returns the number of milliseconds remaining of the current tone or
	 * sample.
	 * 
	 ...
	 * @param aVolume
	 *            The volume of the playback 100 corresponds to 100%
	 */
	...
	static void playSample(File file, int offset, int len, int freq, int vol) {
		// TODO
	}
	...
	public static int playSample(File file, int vol) {
		...
		if (vol >= 0)
			...
			vol = -vol;
		playSample(file, offset, dataLen, sampleRate, vol);
		return getTime();
	}
	 ...
	 * specified volume and sample rate. Returns the number of samples that have
	 * actually been queued.
	 * 
	 * @param data
	 *            Buffer containing the samples
	 * @param offset
	 *            Offset of the first sample in the buffer
	 * @param len
	 *            Number of samples to queue
	 * @param freq
	 *            Sample rate
	 * @param vol
	 *            playback volume
	 ...
	 */
	static int playQueuedSample(byte[] data, int offset, int len, int freq,
			int vol) {
		// TODO
		return 0;
	}
	...
	/**
	 * Queue a series of PCM samples to play at the specified volume and sample
	 * rate. Returns the number of samples that have actually been queued. If
	 * the queue is full, the method exits immediately and returns 0.
	 * 
	 * @param data
	 *            Buffer containing the samples
	 * @param offset
	 ...
	 * @return Number of samples actually queued
	 */
	public static int playSample(byte[] data, int offset, int len, int freq,
			int vol) {
		if (vol >= 0)
			vol = (vol * masterVolume) / 100;
		else
	 ...
	 * automatically to initialize the class. May be called if it is required to
	 * reload any settings.
	 */
