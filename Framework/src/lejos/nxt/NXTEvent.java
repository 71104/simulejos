 ...
 * values changing, button presses etc. To use create a class having the
 * required device type and filter (this may identify a particular port, or I/O
...
public class NXTEvent {
	...
	/**
	 * Register this event with the system. Events must be registered before
	 * they are waited on.
	 * 
	...
	/**
	 * Wait for an event to occur or for the specified timeout. If a timeout
	 * occurs then the TIMEOUT event bit will be set in the result. This bit is
	 * the sign bit so timeouts can be detected by testing for a -ve result.
	 * 
	 * @param timeout
	 *            the timeout in ms. Note a value of <= 0 will return
	 *            immeadiately.
	...
	/**
	 * Wait for an event to occur using the specified filter or for the
	 * specified timeout.
	 * 
	 * @param newFilter
	 *            The type specific filter for this wait.
	 * @param timeout
	 *            the timeout in ms. Note a value of <= 0 will return
	 *            immediately.
	 * @return the event flags or 0 if the event timed out
	 */
	public synchronized int waitEvent(int newFilter, long timeout)
	 ...
	 * 
	 * @param events
	 *            an array of events to wait on.
	 * @param timeout
	 *            the wait timeout. Note a value of <= 0 will return
	 *            immediately.
	...
	/**
	 * This call can be used to raise a user event. User events will wake a
	 * thread from an eventWait call.
	 * 
	 * @param event
	 */
	 ...
	 *            The event type.
	 * @param filter
	 *            The event specific filter.
	 * @param update
	 *            The update period used when checking the event.
	 * @return The new event object.
	 */
	public static synchronized NXTEvent allocate(int type, int filter,
			int update) {
